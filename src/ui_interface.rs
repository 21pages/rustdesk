...
lazy_static::lazy_static! {
    ...
    static ref POLICY_STATUS : Arc<Mutex<PolicyStatus>> = Arc::new(Mutex::new(PolicyStatus{
        remote_modify: false,
    }));
}
                ...
                tokio::select! {
                    res = c.next() => {
                        match res {
                            ...
                            Ok(Some(ipc::Data::PolicyStatus(Some(v)))) => {
                                *POLICY_STATUS.lock().unwrap() = v;
                            }
                            _ => {}
                        }
                    }
                    ...
                    _ = timer.tick() => {
                        ...
                        c.send(&ipc::Data::PolicyStatus(None)).await.ok();
                    }
                }
            }
        }
...
pub fn allow_remote_config_modification() -> bool {
    POLICY_STATUS.lock().unwrap().remote_modify.clone()
}
