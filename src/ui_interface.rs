...
pub struct SessionControlRuleStatus {
...
lazy_static::lazy_static! {
    ...
    static ref SESSION_CONTROL_RULE_STATUS : Arc<Mutex<SessionControlRuleStatus>> = Arc::new(Mutex::new(SessionControlRuleStatus{
        remote_modify: false,
    }));
}
                ...
                tokio::select! {
                    res = c.next() => {
                        match res {
                            ...
                            Ok(Some(ipc::Data::SessionControlRule(Some(v)))) => {
                                *SESSION_CONTROL_RULE_STATUS.lock().unwrap() = v;
                            }
                            _ => {}
                        }
                    }
                    ...
                    _ = timer.tick() => {
                        ...
                        c.send(&ipc::Data::SessionControlRule(None)).await.ok();
                    }
                }
            }
        }
