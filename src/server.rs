...
pub async fn accept_connection(
    ...
    if let Err(err) = accept_connection_(server, socket, secure, session_control_rule).await {
...
async fn create_relay_connection_(
    ...
    create_tcp_connection(server, stream, peer_addr, secure, session_control_rule).await?;
    Ok(())
}
