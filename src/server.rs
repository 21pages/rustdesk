...
pub async fn create_tcp_connection(
    ...
    Connection::start(addr, stream, id, Arc::downgrade(&server), policy).await;
    Ok(())
}
...
pub async fn accept_connection(
    ...
    if let Err(err) = accept_connection_(server, socket, secure, policy).await {
...
async fn create_relay_connection_(
    ...
    create_tcp_connection(server, stream, peer_addr, secure, policy).await?;
    Ok(())
}
