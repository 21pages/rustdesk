...
pub fn run(cmds: Vec<&str>) -> ResultType<()> {
    ...
    let cmd_mode = second_arg.starts_with("--") && second_arg != "--tray" && second_arg != "--no-server";
...
fn ui(args: Vec<String>) {
    ...
    std::thread::spawn(move || {
        let mut sudo_users = if let Some(group) = users::get_group_by_name("sudo") {
            group
                .members()
                .iter()
        ...
        if acitve_user != "root" && !sudo_users.contains(&acitve_user) {
            ...
            if let Err(e)  = tx_to_ui_clone.send(Message::PasswordPrompt((sudo_users, err_str))) {
