...
fn create_process_with_logon_inner(
    user: &str,
    pwd: &str,
    exe: &str,
    arg: &str,
    ...
    unsafe {
        ...
        {
            let last_error = GetLastError();
            if retry_on_access_denied && last_error == ERROR_ACCESS_DENIED {
                log::warn!("CreateProcessWithLogonW access denied, retrying in 500 ms");
                std::thread::sleep(Duration::from_millis(500));
                return create_process_with_logon_inner(user, pwd, exe, arg, false);
            }
            log::error!(
...
pub fn create_process_with_logon(user: &str, pwd: &str, exe: &str, arg: &str) -> ResultType<()> {
    create_process_with_logon_inner(user, pwd, exe, arg, true)
}
