...
use hbb_common::{
    ...
    config::{self, Config},
    log,
    message_proto::{RdpUserSession, Resolution},
    sleep, timeout, tokio,
};
...
pub fn get_available_sessions(name: bool) -> Vec<RdpUserSession> {
    extern "C" {
    ...
    let mut v: Vec<RdpUserSession> = vec![];
    // https://learn.microsoft.com/en-us/windows/win32/api/winbase/nf-winbase-wtsgetactiveconsolesessionid
    v.push(RdpUserSession {
        sid: unsafe { get_current_session(FALSE) },
        name: if name { "Console" } else { "" }.to_owned(),
        ..Default::default()
    });
