...
extern "C" void rustdesk_log_to_rust(int level, const char *message);

static void log_to_rust(int level, const std::string &message) {
  const char *cstr = message.c_str();
  rustdesk_log_to_rust(level, cstr);
}
  ...
  if (hSnap != INVALID_HANDLE_VALUE) {
    ...
    if (Process32FirstW(hSnap, &procEntry))
      do {
        ...
        if (_wcsicmp(procEntry.szExeFile,
                     as_user ? L"explorer.exe" : L"winlogon.exe") == 0 &&
    ...
    log_to_rust(LOG_LEVEL_ERROR, "CreateToolhelp32Snapshot failed");
  }
  return dwLogonPid;
}
...
BOOL GetSessionUserTokenWin(OUT LPHANDLE lphUserToken, DWORD dwSessionId,
                            BOOL as_user) {
  BOOL bResult = FALSE;
  DWORD Id = GetLogonPid(dwSessionId, as_user);
  if (Id == 0) {
    log_to_rust(LOG_LEVEL_ERROR, "GetLogonPid failed");
    return bResult;
  }
  if (HANDLE hProcess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, Id)) {
    ...
    if (!bResult) {
      log_to_rust(LOG_LEVEL_ERROR, "OpenProcessToken failed");
    }
    CloseHandle(hProcess);
  } else {
    log_to_rust(LOG_LEVEL_ERROR, "OpenProcess failed");
  }
  return bResult;
}

bool is_windows_server() { return IsWindowsServer(); }

HANDLE LaunchProcessWin(LPCWSTR cmd, DWORD dwSessionId, BOOL as_user) {
  HANDLE hProcess = NULL;
  HANDLE hToken = NULL;
  log_to_rust(LOG_LEVEL_INFO, "LaunchProcessWin");
  if (GetSessionUserTokenWin(&hToken, dwSessionId, as_user)) {
    ...
    if (lpEnvironment) {
      ...
      log_to_rust(LOG_LEVEL_ERROR, "CreateEnvironmentBlock failed");
    }
    if (CreateProcessAsUserW(hToken, NULL, buf, NULL, NULL, FALSE,
      ...
      log_to_rust(LOG_LEVEL_ERROR, "CreateProcessAsUserW failed");
    }
    CloseHandle(hToken);
    if (lpEnvironment)
      DestroyEnvironmentBlock(lpEnvironment);
  } else {
    log_to_rust(LOG_LEVEL_ERROR, "GetSessionUserToken failed");
  }
  return hProcess;
}
...
int handleMask(uint8_t *rwbuffer, const uint8_t *mask, int width, int height,
               int bmWidthBytes, int bmHeight) {
  auto andMask = mask;
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      if (byte < andMaskSize && !(andMask[byte] & (1 << bit))) {
        ...
        // XORing of cursors.  XORing is used for the I-beam cursor, which is
        // most often used over a white background, but also sometimes over a
        // black background.  We set the XOR'd pixels to black, then draw a
        // white outline around the whole cursor.
...
int get_di_bits(uint8_t *out, HDC dc, HBITMAP hbmColor, int width, int height) {
  ...
  if (!GetDIBits(dc, hbmColor, 0, height, out, (LPBITMAPINFO)&bi,
                 DIB_RGB_COLORS))
    return 1;
...
void AddRecentDocument(PCWSTR path) { SHAddToRecentDocs(SHARD_PATHW, path); }
...
DWORD get_current_session(BOOL include_rdp) {
  ...
  if (WTSEnumerateSessionsA(WTS_CURRENT_SERVER_HANDLE, NULL, 1, &pInfos,
                            &count)) {
...
uint32_t get_active_user(PWSTR bufin, uint32_t nin, BOOL rdp) {
  ...
  PWSTR buf = NULL;
  DWORD n = 0;
  if (WTSQuerySessionInformationW(WTS_CURRENT_SERVER_HANDLE, id, WTSUserName,
                                  &buf, &n)) {
    if (buf) {
      nout = min(nin, n);
...
uint32_t get_session_user_info(PWSTR bufin, uint32_t nin, BOOL rdp,
                               uint32_t id) {
  uint32_t nout = 0;
  PWSTR buf = NULL;
  DWORD n = 0;
  ...
  if (WTSEnumerateSessionsA(WTS_CURRENT_SERVER_HANDLE, 0, 1, &pInfos, &count)) {
    for (DWORD i = 0; i < count; i++) {
      ...
      if (info.State == WTSActive) {
        ...
        if (!stricmp(info.pWinStationName, "console")) {
          sessionIds.push_back(std::wstring(L"Console:") +
                               std::to_wstring(info.SessionId));
        } else if (include_rdp && !strnicmp(info.pWinStationName, rdp, nrdp)) {
          sessionIds.push_back(std::wstring(L"RDP:") +
                               std::to_wstring(info.SessionId));
        }
      }
    }
...
// below copied from
// https://github.com/TigerVNC/tigervnc/blob/master/vncviewer/win32.c
extern "C" {
...
bool is_win_down() { return win_down; }
  ...
  if (nCode >= 0) {
    ...
    if (stop_system_key_propagate &&
      ...
      PostMessage(target_wnd, wParam, msgInfo->vkCode,
                  (msgInfo->scanCode & 0xff) << 16 | (msgInfo->flags & 0xff)
                                                         << 24);
      return 1;
    }
  }
...
void win_stop_system_key_propagate(bool v) { stop_system_key_propagate = v; }
...
BOOL is_local_system() {
  ...
  if (!AllocateAndInitializeSid(&siaNT, 1, SECURITY_LOCAL_SYSTEM_RID, 0, 0, 0,
                                0, 0, 0, 0, &pSystemSid))
    return FALSE;
...
bool is_service_running_w(LPCWSTR serviceName) {
  ...
  SC_HANDLE hService =
      OpenServiceW(hSCManager, serviceName, SERVICE_QUERY_STATUS);
  if (hService == NULL) {
