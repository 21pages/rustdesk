...
pub enum Data {
    ...
    SessionControlRule(Option<crate::ui_interface::SessionControlRuleStatus>),
}
...
async fn handle(data: Data, stream: &mut Connection) {
    match data {
        ...
        Data::SessionControlRule(_) => {
            let remote_modify = crate::server::get_allow_remote_config_modification();
            allow_err!(
                stream
                    .send(&Data::SessionControlRule(Some(
                        crate::ui_interface::SessionControlRuleStatus { remote_modify }
                    )))
                    .await
            );
        }
