    ...
    match data {
        ...
        Data::ControllingStrategy(_) => {
            let disable_remote_modify = {
                use hbb_common::rendezvous_proto::controlling_strategy::Switch;
                let strategies = crate::server::CONTROLLING_STRATEGIES.lock().unwrap();
                strategies.iter().any(|(_, strategy)| {
                    matches!(strategy.remote_modify.enum_value(), Ok(Switch::DISABLE))
                })
            };
            allow_err!(
                stream
                    .send(&Data::ControllingStrategy(Some(
                        crate::ui_interface::ControllingStrategyStatus { disable_remote_modify }
                    )))
                    .await
            );
        }
