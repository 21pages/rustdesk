...
pub fn session_start(
    ...
    session_start_(&session_uuid, &id, events2ui)
}

pub fn session_get_remember(session_uuid: Uuid) -> Option<bool> {
...
pub fn session_get_toggle_option(session_uuid: Uuid, arg: String) -> Option<bool> {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn session_get_toggle_option_sync(session_uuid: Uuid, arg: String) -> SyncReturn<bool> {
    let res = session_get_toggle_option(session_uuid, arg) == Some(true);
    SyncReturn(res)
}

pub fn session_get_option(session_uuid: Uuid, arg: String) -> Option<String> {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn session_close(session_uuid: Uuid) {
    if let Some(mut session) = SESSIONS.write().unwrap().remove(&session_uuid) {
        session.close_event_stream();
        session.close();
    }
}

pub fn session_refresh(session_uuid: Uuid) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        session.refresh_video();
    }
}

pub fn session_record_screen(session_uuid: Uuid, start: bool, width: usize, height: usize) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        session.record_screen(start, width as _, height as _);
    }
}

pub fn session_reconnect(session_uuid: Uuid, force_relay: bool) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        session.reconnect(force_relay);
    }
}

pub fn session_toggle_option(session_uuid: Uuid, value: String) {
    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    if SESSIONS.read().unwrap().get(&session_uuid).is_some() && value == "disable-clipboard" {
        crate::flutter::update_text_clipboard_required();
    }
}

pub fn session_get_flutter_config(session_uuid: Uuid, k: String) -> Option<String> {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn session_set_flutter_config(session_uuid: Uuid, k: String, v: String) {
    if let Some(session) = SESSIONS.write().unwrap().get_mut(&session_uuid) {
...
pub fn session_get_view_style(session_uuid: Uuid) -> Option<String> {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn session_set_view_style(session_uuid: Uuid, value: String) {
    if let Some(session) = SESSIONS.write().unwrap().get_mut(&session_uuid) {
        session.save_view_style(value);
    }
}

pub fn session_get_scroll_style(session_uuid: Uuid) -> Option<String> {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn session_set_scroll_style(session_uuid: Uuid, value: String) {
    if let Some(session) = SESSIONS.write().unwrap().get_mut(&session_uuid) {
        session.save_scroll_style(value);
    }
}

pub fn session_get_image_quality(session_uuid: Uuid) -> Option<String> {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn session_set_image_quality(session_uuid: Uuid, value: String) {
    if let Some(session) = SESSIONS.write().unwrap().get_mut(&session_uuid) {
        session.save_image_quality(value);
    }
}

pub fn session_get_keyboard_mode(session_uuid: Uuid) -> Option<String> {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn session_set_keyboard_mode(session_uuid: Uuid, value: String) {
...
pub fn session_get_custom_image_quality(session_uuid: Uuid) -> Option<Vec<i32>> {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn session_is_keyboard_mode_supported(session_uuid: Uuid, mode: String) -> SyncReturn<bool> {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn session_set_custom_image_quality(session_uuid: Uuid, value: i32) {
    if let Some(session) = SESSIONS.write().unwrap().get_mut(&session_uuid) {
        session.save_custom_image_quality(value);
    }
}

pub fn session_set_custom_fps(session_uuid: Uuid, fps: i32) {
    if let Some(session) = SESSIONS.write().unwrap().get_mut(&session_uuid) {
        session.set_custom_fps(fps);
    }
}

pub fn session_lock_screen(session_uuid: Uuid) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        session.lock_screen();
    }
}

pub fn session_ctrl_alt_del(session_uuid: Uuid) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        session.ctrl_alt_del();
    }
}

pub fn session_switch_display(session_uuid: Uuid, value: i32) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn session_enter_or_leave(_session_uuid: Uuid, _enter: bool) {
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    if let Some(session) = SESSIONS.read().unwrap().get(&_session_uuid) {
        if _enter {
            session.enter();
        } else {
            session.leave();
...
pub fn session_input_string(session_uuid: Uuid, value: String) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        // #[cfg(any(target_os = "android", target_os = "ios"))]
...
// chat_client_mode
pub fn session_send_chat(session_uuid: Uuid, text: String) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        session.send_chat(text);
    }
}

pub fn session_peer_option(session_uuid: Uuid, name: String, value: String) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        session.set_option(name, value);
    }
}

pub fn session_get_peer_option(session_uuid: Uuid, name: String) -> String {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn session_input_os_password(session_uuid: Uuid, value: String) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
// File Action
pub fn session_read_remote_dir(session_uuid: Uuid, path: String, include_hidden: bool) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn session_cancel_job(session_uuid: Uuid, act_id: i32) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        session.cancel_job(act_id);
    }
}

pub fn session_create_dir(session_uuid: Uuid, act_id: i32, path: String, is_remote: bool) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        session.create_dir(act_id, path, is_remote);
    }
}

pub fn session_read_local_dir_sync(_session_uuid: Uuid, path: String, show_hidden: bool) -> String {
...
pub fn session_get_platform(session_uuid: Uuid, is_remote: bool) -> String {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn session_load_last_transfer_jobs(session_uuid: Uuid) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn session_resume_job(session_uuid: Uuid, act_id: i32, is_remote: bool) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        session.resume_job(act_id, is_remote);
    }
}

pub fn session_elevate_direct(session_uuid: Uuid) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        session.elevate_direct();
    }
}

pub fn session_elevate_with_logon(session_uuid: Uuid, username: String, password: String) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        session.elevate_with_logon(username, password);
    }
}

pub fn session_switch_sides(session_uuid: Uuid) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        session.switch_sides();
    }
}

pub fn session_change_resolution(session_uuid: Uuid, width: i32, height: i32) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        session.change_resolution(width, height);
    }
}

pub fn session_set_size(_session_uuid: Uuid, _width: usize, _height: usize) {
    #[cfg(feature = "flutter_texture_render")]
    if let Some(session) = SESSIONS.write().unwrap().get_mut(&_session_uuid) {
...
pub fn session_remove_port_forward(session_uuid: Uuid, local_port: i32) {
    if let Some(session) = SESSIONS.write().unwrap().get_mut(&session_uuid) {
        session.remove_port_forward(local_port);
    }
}

pub fn session_new_rdp(session_uuid: Uuid) {
    if let Some(session) = SESSIONS.write().unwrap().get_mut(&session_uuid) {
        session.new_rdp();
    }
}

pub fn session_request_voice_call(session_uuid: Uuid) {
    if let Some(session) = SESSIONS.write().unwrap().get_mut(&session_uuid) {
        session.request_voice_call();
    }
}

pub fn session_close_voice_call(session_uuid: Uuid) {
    if let Some(session) = SESSIONS.write().unwrap().get_mut(&session_uuid) {
...
pub fn session_send_mouse(session_uuid: Uuid, msg: String) {
...
pub fn session_restart_remote_device(session_uuid: Uuid) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        session.restart_remote_device();
    }
}

pub fn session_get_audit_server_sync(session_uuid: Uuid, typ: String) -> SyncReturn<String> {
    let res = if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn session_send_note(session_uuid: Uuid, note: String) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
        session.send_note(note)
    }
}

pub fn session_alternative_codecs(session_uuid: Uuid) -> String {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn session_change_prefer_codec(session_uuid: Uuid) {
    if let Some(session) = SESSIONS.read().unwrap().get(&session_uuid) {
...
pub fn set_cur_session_id(session_uuid: Uuid) {
    super::flutter::set_cur_session_id(session_uuid);
    #[cfg(windows)]
    crate::keyboard::update_grab_get_key_name();
}
