...
use hbb_common::{
    ...
    bytes::Bytes,
    config::{
        self, keys::OPTION_ENABLE_WEBSOCKET, option2bool, Config, CONNECT_TIMEOUT, READ_TIMEOUT,
        RENDEZVOUS_PORT, WS_RELAY_PORT, WS_RENDEZVOUS_PORT,
    },
...
pub fn is_ws_config(server: bool) -> bool {
    let option = OPTION_ENABLE_WEBSOCKET;
    let v = if server {
        Config::get_option(option)
    } else {
        ui_interface::get_option(option)
    };
    option2bool(option, &v)
}
...
mod tests {
    ...
    fn test_check_ws() {
        ...
        Config::set_option(OPTION_ENABLE_WEBSOCKET.to_owned(), "".to_owned());
        ...
        Config::set_option(OPTION_ENABLE_WEBSOCKET.to_owned(), "Y".to_owned());
        assert_eq!(
