...
impl RendezvousMediator {
    ...
    async fn handle_request_relay(&self, rr: RequestRelay, server: ServerPtr) -> ResultType<()> {
        ...
        *LAST_RELAY_MSG.lock().await = (addr, Instant::now());
        ...
        self.create_relay(
            ...
            false,
            Default::default(),
            rr.session_control_rule.clone().into_option(),
        )
        .await
    }
    ...
    async fn create_relay(
        ...
        session_control_rule: Option<SessionControlRule>,
    ) -> ResultType<()> {
    ...
    async fn handle_intranet(&self, fla: FetchLocalAddr, server: ServerPtr) -> ResultType<()> {
        ...
        *LAST_MSG.lock().await = (addr, Instant::now());
        ...
        let session_control_rule = fla.session_control_rule.clone().into_option();
        if peer_addr_v6.port() > 0 && !relay {
            socket_addr_v6 = start_ipv6(
                peer_addr_v6,
                addr,
                server.clone(),
                session_control_rule.clone(),
            )
            .await;
        }
    ...
    async fn handle_intranet_(
        ...
        crate::accept_connection(
            ...
            peer_addr,
            true,
            fla.session_control_rule.into_option(),
        )
        .await;
        Ok(())
    }
    ...
    async fn handle_punch_hole(&self, ph: PunchHole, server: ServerPtr) -> ResultType<()> {
        ...
        *LAST_MSG.lock().await = (peer_addr, Instant::now());
        ...
        let mut socket_addr_v6 = Default::default();
        // Extract permission session_control_rule from PunchHole message
        let session_control_rule = ph.session_control_rule.into_option();
        if peer_addr_v6.port() > 0 && !relay {
            socket_addr_v6 = start_ipv6(
                peer_addr_v6,
                peer_addr,
        ...
        if ph.udp_port > 0 {
            ...
            self.punch_udp_hole(peer_addr, server, msg_punch, session_control_rule)
                .await?;
            return Ok(());
        }
                ...
                tokio::spawn(async move {
                    allow_err!(
                        crate::server::create_tcp_connection(
                            ...
                            addr,
                            false,
                            None, // IPv6 direct connections don't have permission session_control_rule
                        )
                        .await
                    );
                });
