...
pub const USE_VIDEO_ACK: bool = false; // false;
...
impl VideoQoS {
    pub fn remove_display(&mut self, display_idx: usize) {
        self.display_fps.remove(&display_idx);
    }

    pub fn update_display_fps(&mut self, display_idx: usize, fps: u32) {
        self.display_fps.insert(display_idx, fps);
    ...
    fn adjust_ratio(&mut self) {
        ...
        for (_, user) in self.users.iter() {
            ...
            if let Some(rtt) = user.delay.rtt {
                ...
                if !delay_history.is_empty() {
                    let avg_delay =
                        delay_history.iter().sum::<u32>() as f32 / delay_history.len() as f32;
        ...
        // Adjust based on delay thresholds
        if max_delay > 200.0 {
            // High delay: aggressive reduction
            v = current_ratio * 0.85;
            println!(
                "High delay ({:.0}ms), reducing quality significantly",
                max_delay
            );
        } else if max_delay > 100.0 {
            // Medium delay: moderate reduction
            v = current_ratio * 0.9;
            println!("Medium delay ({:.0}ms), reducing quality", max_delay);
        } else if max_delay < 50.0 {
            // Low delay: can try increasing quality
            match target_quality {
                Quality::Best => {
                    if current_ratio < target_ratio {
                        v = (current_ratio * 1.1).min(target_ratio);
                    }
                }
                Quality::Balanced | Quality::Custom(_) => {
                    if current_ratio < BR_BALANCED {
                        v = (current_ratio * 1.05).min(BR_BALANCED);
                    }
                }
                Quality::Low => {
                    if current_ratio < BR_SPEED {
                        v = (current_ratio * 1.05).min(BR_SPEED);
                    }
                }
            }
            println!("Low delay ({:.0}ms), can increase quality", max_delay);
        }
        ...
        println!(
            "after adjust - ratio: {:.2}, max_delay: {:.1}ms, quality: {:?}",
            self.ratio, max_delay, target_quality
        );
    }
}
...
impl VideoQoS {
    ...
    fn adjust_fps_based_on_ack(&mut self) {
        ...
        for (_, fps) in self.display_fps.iter() {
