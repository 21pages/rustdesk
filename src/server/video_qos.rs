...
pub const USE_VIDEO_ACK: bool = true;
...
impl VideoQoS {
    ...
    pub fn update_display_fps(&mut self, display_idx: usize, fps: u32, send_counter: usize) {
        println!(
            "update_display_fps: {:?}, self.displays: {:?}",
            display_idx, self.displays
        );
        if let Some(display) = self.displays.get_mut(&display_idx) {
            display.fps = fps;
        ...
        if abr_enabled {
            if self.adjust_ratio_instant.elapsed().as_secs() >= ADJUST_RATIO_INTERVAL as u64 {
                ...
                self.displays.iter_mut().for_each(|d| {
                    println!(
                        "send_counter: {:?}, dynamic_screen: {}",
                        d.1.send_counter, dynamic_screen
                    );
                    d.1.send_counter = 0;
                });
    ...
    fn adjust_fps_based_on_ratio(&mut self) {
        ...
        if target_ratio >= self.br_best() {
            if current_ratio < self.br_best() * 0.95 {
                if current_ratio > self.br_balanced() {
                    if current_fps > 22 {
                        self.fps = 22.min(current_fps - 5);
                    }
                } else if current_ratio > self.br_speed() {
                    if current_fps > 18 {
            ...
            if current_ratio < self.br_balanced() * 0.95 {
                if current_ratio > self.br_speed() {
                    if current_fps > 22 {
                        self.fps = 22.min(current_fps - 5);
                    }
                } else {
        ...
        } else if target_ratio >= self.br_speed() {
            if current_ratio < self.br_speed() * 0.9 {
                if current_fps > 25 {
    ...
    fn adjust_ratio(&mut self, dynamic_screen: bool) {
        ...
        println!(
            "after adjust - current_ratio: {:.2}, ratio: {:.2}, max_delay: {:.1}ms, quality: {:?}, fps: {:?}, min: {:.2}, max: {:.2}, dynamic_screen: {}",
            current_ratio, self.ratio, max_delay, target_quality, self.fps, min, max, dynamic_screen
        );
    }
}
...
impl VideoQoS {
    pub fn user_network_delay(&mut self, id: i32, delay: u32) {
        ...
        if let Some(user) = self.users.get_mut(&id) {
            ...
            let mut avg_delay = if len > 0 {
                user.delay.delay_history.iter().sum::<u32>() as f32 / len as f32
            } else {
                delay as f32
            };
            if len >= HISTORY_DELAY_LEN {
                avg_delay -= user.delay.rtt.unwrap_or_default() as f32
            }
            avg_delay = avg_delay.max(10.0);
            let fps = (2000.0 / avg_delay).ceil() as u32;
            user.delay.fps = Some(fps.clamp(MIN_FPS, highest_fps));
            // println!(
            //     "avg_delay: {:?}, rtt: {:?},  fps: {:?}",
            //     avg_delay, user.delay.rtt, user.delay.fps
            // );
        }
        self.adjust_fps_based_on_delay();
    }
...
impl VideoHistory {
    ...
    fn congested(&mut self) -> bool {
        ...
        let fps = qos.fps();
        let highest_fps = qos.highest_fps();
        drop(qos);
        let fps = (highest_fps as f32 * 0.2 + fps as f32 * 0.8).ceil() as u32;
        let fps = fps.clamp(MIN_FPS, MAX_FPS);
        let ms_per_frame = 1000. / fps as f64;
