...
impl Connection {
    pub async fn start(
        ...
        let mut conn = Self {
            ...
            video_history: VideoHistory::new(id),
        };
        ...
        loop {
            tokio::select! {
                ...
                Some((instant, value)) = rx_video.recv() => {
                    ...
                    conn.video_history.on_send(value.compute_size() as _);
                ...
                _ = test_delay_timer.tick() => {
                    ...
                    if conn.last_test_delay.is_none() && !(conn.port_forward_socket.is_some() && conn.authorized) {
                        ...
                        let mut msg_out = Message::new();
                        let time = conn.video_history.timestamp();
    ...
    impl AuthedConnID {
        pub fn new(conn_id: i32, conn_type: AuthConnType, session_key: SessionKey) -> Self {
            ...
            if conn_type == AuthConnType::Remote {
                VIDEO_QOS.lock().unwrap().on_connection_open(conn_id);
            }
