...
use hbb_common::{
    ...
    rendezvous_proto::session_control_rule::Switch,
    sha2::{Digest, Sha256},
    sleep, timeout,
    tokio::{
        net::TcpStream,
...
lazy_static::lazy_static! {
    ...
    static ref SessionControlRules: Arc::<Mutex<Vec<(i32, SessionControlRule)>>> = Default::default();
...
pub struct Connection {
    ...
    recording: bool,
    block_input: bool,
    rule: Option<SessionControlRule>,
...
impl Connection {
    pub async fn start(
        ...
        let _raii_policy_id = raii::SessionControlRuleID::new(id, &rule);
        let hash = Hash {
        ...
        let mut conn = Self {
            ...
            tx_to_cm,
            authorized: false,
            keyboard: Self::permission(keys::OPTION_ENABLE_KEYBOARD, &rule),
            clipboard: Self::permission(keys::OPTION_ENABLE_CLIPBOARD, &rule),
            audio: Self::permission(keys::OPTION_ENABLE_AUDIO, &rule),
            // to-do: make sure is the option correct here
            file: Self::permission(keys::OPTION_ENABLE_FILE_TRANSFER, &rule),
            restart: Self::permission(keys::OPTION_ENABLE_REMOTE_RESTART, &rule),
            recording: Self::permission(keys::OPTION_ENABLE_RECORD_SESSION, &rule),
            block_input: Self::permission(keys::OPTION_ENABLE_BLOCK_INPUT, &rule),
            rule,
                ...
                Some(data) = rx_from_authed.recv() => {
                    match data {
                        ...
                        ipc::Data::PrinterData(data) => {
                            if Self::permission(keys::OPTION_ENABLE_REMOTE_PRINTER, &conn.rule) {
    ...
    fn permission(enable_prefix_option: &str, policy: &Option<SessionControlRule>) -> bool {
        ...
        if let Some(message::Union::LoginRequest(lr)) = msg.union {
            ...
            match lr.union {
                Some(login_request::Union::FileTransfer(ft)) => {
                    if !Self::permission(keys::OPTION_ENABLE_FILE_TRANSFER, &self.rule) {
                ...
                Some(login_request::Union::ViewCamera(_vc)) => {
                    if !Self::permission(keys::OPTION_ENABLE_CAMERA, &self.rule) {
                ...
                Some(login_request::Union::Terminal(terminal)) => {
                    if !Self::permission(keys::OPTION_ENABLE_TERMINAL, &self.rule) {
                ...
                Some(login_request::Union::PortForward(mut pf)) => {
                    if !Self::permission(keys::OPTION_ENABLE_TUNNEL, &self.rule) {
...
mod raii {
    ...
    // SESSION_CONTROL_RULE: all non-None session control rules
    ...
    pub struct SessionControlRuleID {
        id: i32,
        rule: SessionControlRule,
    }

    impl Drop for SessionControlRuleID {
        fn drop(&mut self) {
            let mut lock = SessionControlRules.lock().unwrap();
            lock.retain(|(conn_id, _)| *conn_id != self.id);
        }
    }

    impl SessionControlRuleID {
        pub fn new(id: i32, rule: &Option<SessionControlRule>) -> Option<Self> {
            let Some(rule) = rule else {
                return None;
            };
            SessionControlRules.lock().unwrap().push((id, rule.clone()));
            Some(Self {
                id,
                rule: rule.clone(),
            })
...
pub fn get_allow_remote_config_modification() -> bool {
    ...
    let rules = SessionControlRules.lock().unwrap();
    for (_, rule) in rules.iter() {
        match rule.remote_modify.enum_value() {
