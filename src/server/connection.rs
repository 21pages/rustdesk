...
lazy_static::lazy_static! {
    ...
    pub static ref AUTHED_CONNS: Arc::<Mutex<Vec<(i32, AuthConnType, String, u64)>>> = Default::default();
...
impl Connection {
            ...
            if self.validate_one_password(password.clone()) {
                raii::AuthedConnID::insert_session(
                    ...
                    Session {
                        ...
                        tfa: false,
                        conn_type: self.get_conn_type_before_authed(),
    ...
    fn is_recent_session(&mut self, tfa: bool) -> bool {
        ...
        if let Some(mut session) = session {
                    ...
                    || !tfa && self.validate_one_password(session.random_password.clone()))
            {
                ...
                session.conn_type = self.get_conn_type_before_authed();
    ...
    #[inline]
    fn get_conn_type_before_authed(&self) -> AuthConnType {
        if self.file_transfer.is_some() {
    ...
    impl AuthedConnID {
        pub fn new(
            ...
            AUTHED_CONNS
                .lock()
                .unwrap()
                .push((conn_id, conn_type, peer_id.clone(), session_id));
        ...
        pub fn remove_session_if_last_duplication(conn_id: i32, key: SessionKey) {
            ...
            if contains {
                let another = AUTHED_CONNS.lock().unwrap().iter().any(|c| {
                    c.0 != conn_id
                        && c.2 == key.peer_id
                        && c.3 == key.session_id
                        && c.1 != AuthConnType::PortForward
                });
                if !another {
            ...
            if session.conn_type == AuthConnType::PortForward {
                ...
                let other_alive_conns = AUTHED_CONNS
                    ...
                    .filter(|c| {
                        c.2 == key.peer_id
                            && c.3 == key.session_id
                            && c.1 != AuthConnType::PortForward // port forward doesn't remove itself
                    })
                    .map(|c| c.0)
                    .collect::<Vec<_>>();
