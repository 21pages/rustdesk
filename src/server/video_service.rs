...
fn run(vs: VideoService) -> ResultType<()> {
    ...
    #[cfg(windows)]
    let hdr_enabled = vs.source.is_monitor() && c.hdr_status().enabled;
    #[cfg(windows)]
    if vs.source.is_monitor() && !last_portable_service_running && !c.is_gdi() && hdr_enabled {
        if !c.set_hdr_capture(true) {
            log::warn!("failed to enable HDR capture, continue with SDR capture");
        }
    }
    #[cfg(windows)]
    if vs.source.is_monitor() {
        log::info!(
            "======== HDR to SDR capture: display={}, status={:?}, gdi={}, portable={}, codec={:?}",
            display_idx,
            c.hdr_status(),
            c.is_gdi(),
