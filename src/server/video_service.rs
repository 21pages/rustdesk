...
use scrap::{
    ...
    codec::{Encoder, EncoderCfg, HwEncoderConfig, TexEncoderConfig},
...
fn run(sp: GenericService) -> ResultType<()> {
    ...
    let encoder_cfg = match negotiated_codec.clone() {
        scrap::CodecName::H264(name) | scrap::CodecName::H265(name) => {
            match scrap::texcodec::TexEncoder::try_get(&c.device(), negotiated_codec.clone()) {
                Some(feature) => EncoderCfg::TEX(TexEncoderConfig {
                    device: c.device(),
                    width: c.width,
                    height: c.height,
                ...
                None => EncoderCfg::HW(HwEncoderConfig {
                    name: name.clone(),
                    width: c.width,
                    height: c.height,
                    bitrate: bitrate as _,
