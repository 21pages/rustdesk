...
impl Drop for RunCleaner {
    fn drop(&mut self) {
        #[cfg(feature = "gpucodec")]
        {
            use scrap::gpucodec::GpuEncoder;
            GpuEncoder::set_video_service_adapter_luid(None);
            GpuEncoder::set_this_time_not_use(false);
        }
    }
}
...
fn get_encoder_config(
    ...
    if _portable_service || c.is_gdi() {
        ...
        scrap::gpucodec::GpuEncoder::set_this_time_not_use(true);
    }
    #[cfg(feature = "gpucodec")]
    {
        scrap::gpucodec::GpuEncoder::set_video_service_adapter_luid(Some(c.device().luid));
    ...
    match negotiated_codec.clone() {
        scrap::CodecName::H264GPU | scrap::CodecName::H265GPU => {
            #[cfg(feature = "gpucodec")]
            if let Some(feature) =
                scrap::gpucodec::GpuEncoder::try_get(&c.device(), negotiated_codec.clone())
            {
                EncoderCfg::GPU(scrap::codec::GpuEncoderConfig {
                    device: c.device(),
                    width: c.width,
                    height: c.height,
                    quality,
        ...
        scrap::CodecName::H264HW(_name) | scrap::CodecName::H265HW(_name) => handle_hw_encoder(
            negotiated_codec.clone(),
            c.width,
            c.height,
            quality as _,
        ...
        match _name {
            CodecName::H264GPU | CodecName::H265GPU => {
                ...
                if let Some(h264) = best.h264 {
                    if !is_h265 {
                        Encoder::fallback(CodecName::H264HW(h264.name.clone()));
                ...
                if let Some(h265) = best.h265 {
                    if is_h265 {
                        Encoder::fallback(CodecName::H265HW(h265.name.clone()));
                        return Ok(EncoderCfg::HW(scrap::codec::HwEncoderConfig {
                            name: h265.name,
                            width,
                            height,
