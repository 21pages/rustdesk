...
fn run(vs: VideoService) -> ResultType<()> {
    ...
    let (mut fps_counter, mut fps_instant) = (0, Instant::now());
    while sp.ok() {
        #[cfg(windows)]
        ...
        if check_congestion(display_idx, spf, &mut fps_counter, &mut fps_instant) {
            continue;
        }
    ...
    if fps_instant.elapsed() > Duration::from_secs(1) {
        *fps_instant = Instant::now();
        VIDEO_QOS
            .lock()
            .unwrap()
            .update_display_fps(display_idx, *fps_counter);
        *fps_counter = 0;
    }
    congested
}
