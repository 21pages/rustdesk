...
impl PasswordSource {
    ...
    pub fn is_shared_ab(&self, password: &[u8], hash: &Hash) -> bool {
        ...
        match self {
            PasswordSource::SharedAb(p) => true,
            _ => false,
        }
    }
...
impl LoginConfigHandler {
    ...
    pub fn initialize(
        ...
        if let Some(shared_password) = shared_password {
            ...
            hasher.update(salt);
            let res = hasher.finalize();
            self.shared_password = Some(SharedAbPassword::Hash(res[..].into(), salt.to_owned()));
        }
    ...
    fn create_login_msg(
        ...
        os_password: String,
        password: Vec<u8>,
        shared_password_salt: Option<String>,
    ) -> Message {
        ...
        let mut lr = LoginRequest {
            ...
            salt: shared_password_salt.unwrap_or_default(),
            my_id,
...
pub async fn handle_hash(
    ...
    let mut shared_password_salt = None;
    if let Some(shared_password) = shared_password {
        match shared_password.clone() {
            ...
            SharedAbPassword::Hash(p, salt) => {
                if hash.support_controlling_salt && !p.is_empty() && !salt.is_empty() {
                    ...
                    let res = hasher.finalize();
                    password = res[..].into();
                    shared_password_salt = Some(salt);
                    lc.write().unwrap().password_source = PasswordSource::SharedAb(shared_password);
                }
            }
        }
