            ...
            video_frame::Union::H264s(h264s) => {
                #[cfg(feature = "texcodec")]
                {
                    if let Some(decoder) = &mut self.tex.h264 {
                        *pixelbuffer = false;
                        return Decoder::handle_tex_video_frame(decoder, h264s, texture);
                    }
                }
                #[cfg(feature = "hwcodec")]
                {
            ...
            video_frame::Union::H265s(h265s) => {
                #[cfg(feature = "texcodec")]
                {
                    if let Some(decoder) = &mut self.tex.h265 {
                        *pixelbuffer = false;
                        return Decoder::handle_tex_video_frame(decoder, h265s, texture);
                    }
                }
                #[cfg(feature = "hwcodec")]
                {
