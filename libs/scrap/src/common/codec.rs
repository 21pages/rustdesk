...
use crate::{gpucodec::*, AdapterDevice};
...
impl Encoder {
    pub fn new(config: EncoderCfg) -> ResultType<Encoder> {
        ...
        match config {
            ...
            #[cfg(feature = "gpucodec")]
            EncoderCfg::GPU(_) => match GpuEncoder::new(config) {
                Ok(tex) => Ok(Encoder {
                    codec: Box::new(tex),
                }),
                Err(e) => {
    ...
    pub fn update(update: EncodingUpdate) {
        ...
        if enable_gpucodec_option() {
            if _all_support_h264_decoding {
                if GpuEncoder::possible_available(CodecName::H264GPU).len() > 0 {
                    h264gpu_encoding = true;
                }
            }
            if _all_support_h265_decoding {
                if GpuEncoder::possible_available(CodecName::H265GPU).len() > 0 {
    ...
    pub fn supported_encoding() -> SupportedEncoding {
        ...
        #[cfg(feature = "gpucodec")]
        if enable_gpucodec_option() {
            encoding.h264 |= GpuEncoder::possible_available(CodecName::H264GPU).len() > 0;
            encoding.h265 |= GpuEncoder::possible_available(CodecName::H265GPU).len() > 0;
        }
        encoding
    }
...
impl Decoder {
        ...
        #[cfg(feature = "gpucodec")]
        {
            if enable_gpucodec_option() && _flutter {
                decoding.ability_h264 |=
                    if GpuDecoder::possible_available(CodecName::H264GPU, _luid).len() > 0 {
                        1
                    } else {
                        0
                    };
                decoding.ability_h265 |=
                    if GpuDecoder::possible_available(CodecName::H265GPU, _luid).len() > 0 {
                        1
                    } else {
                        0
                    };
    ...
    pub fn new(_luid: i64) -> Decoder {
        ...
        Decoder {
            ...
            #[cfg(feature = "gpucodec")]
            gpu: if enable_gpucodec_option() && _luid != 0 {
