...
const AV1_1080P: &[u8] = include_bytes!("../../res/1080p.av1");
...
pub fn test_av1() {
    ...
    ONCE.call_once(|| {
        let f = || {
            ...
            let Ok(mut decoder) = AomDecoder::new() else {
                return false;
            };
            let mut key_frame = aom::Image::new();
            let Ok(frames) = decoder.decode(AV1_1080P) else {
                return false;
            };
            for frame in frames {
                if !frame.is_null() {
            ...
            if key_frame.is_null() {
                let Ok(frames) = decoder.flush() else {
                    return false;
                };
                for frame in frames {
                    if !frame.is_null() {
            ...
            let dst_fmt = av1.yuvfmt();

            let dst_stride_y = dst_fmt.stride[0];
            let dst_stride_uv = dst_fmt.stride[1];
            let mut yuv = vec![0u8; (dst_fmt.h * dst_stride_y * 2) as usize];
            let dst_y = yuv.as_mut_ptr();
            let dst_u = yuv[dst_fmt.u..].as_mut_ptr();
            let dst_v = yuv[dst_fmt.v..].as_mut_ptr();
            let res = unsafe {
                crate::ARGBToI420(
                    rgb.raw.as_ptr(),
                    (width * 4) as _,
