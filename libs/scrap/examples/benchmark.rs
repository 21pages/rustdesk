...
mod hw {
    use super::*;
    use hw_common::{
        DataFormat, DecodeContext, DynamicContext, EncodeContext, FeatureContext, PixelFormat,
        MAX_GOP,
    };
    use hwcodec::{
        decode::{self, Decoder},
    ...
    pub fn test(
        ...
        let d = DynamicContext {
            width,
            height,
            kbitrate: kbitrate as _,
            framerate: 30,
            gop: MAX_GOP as _,
        };
        let encoders = encode::available(d.clone());
        println!("hw encoders: {}", encoders.len());
        let best = encode::Best::new(encoders.clone());
        for ctx in encoders {
            test_encoder(ctx.clone(), yuvs, is_best_encoder(&best, &ctx), d.clone());
        }

        let (h264s, h265s) = prepare_h26x(best, yuvs, d);
    ...
    fn test_encoder(f: FeatureContext, yuvs: &Vec<Vec<u8>>, best: bool, d: DynamicContext) {
        let ctx = EncodeContext { f, d };
        let mut encoder = Encoder::new(ctx.clone()).unwrap();
        let start = Instant::now();
        for yuv in yuvs {
            let (datas, linesizes) = split_yuv(ctx.f.pixfmt, ctx.d.width, ctx.d.height, &yuv);
    ...
    fn prepare_h26x(
        ...
        let f = |ctx: Option<FeatureContext>| {
            let mut h26xs = vec![];
            if let Some(ctx) = ctx {
                let ctx = EncodeContext { f: ctx, d };
                let mut encoder = Encoder::new(ctx.clone()).unwrap();
                for yuv in yuvs {
                    let (datas, linesizes) =
                        split_yuv(ctx.f.pixfmt, ctx.d.width, ctx.d.height, &yuv);
    ...
    fn is_best_encoder(best: &encode::Best, ctx: &FeatureContext) -> bool {
