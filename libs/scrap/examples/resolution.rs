...
mod hw {
    ...
    pub fn test(bgra: &Vec<u8>, width: i32, height: i32, pixfmt: PixelFormat, dir: &PathBuf) {
        ...
        let mut encoders = encode::available(d.clone());
        println!("hw encoders: {}", encoders.len());
        let best = encode::Best::new(encoders.clone());
        for ctx in encoders {
            test_encoder(ctx.clone(), &yuv, dir, d);
        }

        let (h264, h265) = prepare_h26x(best, &yuv, d.clone());
    ...
    fn test_encoder(f: FeatureContext, yuv: &Vec<u8>, dir: &PathBuf, d: DynamicContext) {
        println!("{:?} {:?} {:?} ", f.driver, f.dataFormat, f.device);
        ...
        let filename = format!(
            ...
            ctx.f.driver,
            ctx.f.device,
            if ctx.f.dataFormat == DataFormat::H264 {
                "264"
            } else {
                "265"
            }
    ...
    fn prepare_h26x(best: encode::Best, yuv: &Vec<u8>, d: DynamicContext) -> (Vec<u8>, Vec<u8>) {
        let f = |f: Option<FeatureContext>| {
            if let Some(f) = f {
                let ctx = EncodeContext { f, d };
                let mut encoder = Encoder::new(ctx.clone()).unwrap();
                let (datas, linesizes) = split_yuv(ctx.f.pixfmt, ctx.d.width, ctx.d.height, &yuv);
