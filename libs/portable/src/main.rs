...
fn log_to_file(s: &str) {
    #[cfg(windows)]
    {
        let mut file = std::fs::File::create("portable.log").unwrap();
        let _ = std::io::Write::write_all(&mut file, s.as_bytes());
    }
}
...
fn execute(path: PathBuf, args: Vec<String>, _ui: bool) {
    ...
    if use_null_stdio() {
        println!("Using null stdio (Win7 workaround), not set");
        log_to_file("Using null stdio (Win7 workaround), not set");
    } else {
