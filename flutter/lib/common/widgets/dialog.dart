...
customImageQualityDialog(SessionID sessionId, String id, FFI ffi) async {
  int qualityInitValue = kDefaultQuality;
  bool? direct;
  try {
    direct =
  ...
  setValue(int quality) async {
    await bind.sessionSetCustomImageQuality(
        sessionId: sessionId, value: quality);
    debugPrint("set custom quality:$quality");
  }
  ...
  qualityInitValue =
      quality != null && quality.isNotEmpty ? quality[0] : kDefaultQuality;

  final content = customImageQualityWidget(
      initQuality: qualityInitValue,
      setQuality: (v) => setValue(v),
      showMoreQuality: !hideMoreQuality);
  msgBoxCommon(ffi.dialogManager, 'Custom Image Quality', content, [btnClose]);
}

customFpsDialog(SessionID sessionId, String id, FFI ffi, RxInt rxFps) async {
  setValue(int fps) async {
    await bind.sessionSetCustomFps(sessionId: sessionId, fps: fps);
    debugPrint("set custom fps:$fps");
  }

  final btnClose = dialogButton('Close', onPressed: () async {
    ffi.dialogManager.dismissAll();
  });
  final content = customFpsWidget(
    rxFps: rxFps,
    setFps: (v) => setValue(v),
  );
  msgBoxCommon(ffi.dialogManager, 'Max FPS', content, [btnClose]);
}
